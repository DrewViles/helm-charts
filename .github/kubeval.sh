...
for CHART_DIR in ${CHART_DIRS}; do
  helm template --values charts/"${CHART_DIR}"/values.yaml charts/"${CHART_DIR}" | ./kubeval --strict --ignore-missing-schemas --kubernetes-version "${KUBERNETES_VERSION#v}" --schema-location "${SCHEMA_LOCATION}"
done
